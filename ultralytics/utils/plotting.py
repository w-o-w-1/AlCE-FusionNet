...
def feature_visualization(x, module_type, stage, n=32, save_dir=Path("runs/detect/exp")):
    ...
    if isinstance(x, torch.Tensor):
        ...
        if height > 1 and width > 1:
            ...
            # 将所有通道的特征图叠加
            combined_feature_map = torch.sum(x[0], dim=0).cpu().detach().numpy()  # shape: (height, width)

            # 标准化到 [0, 1] 范围
            combined_feature_map = (combined_feature_map - np.min(combined_feature_map)) / (
                        np.max(combined_feature_map) - np.min(combined_feature_map))

            # 转换为 8-bit 图像并应用颜色映射
            combined_feature_map = np.array(combined_feature_map * 255.0, dtype=np.uint8)
            combined_feature_map = cv2.applyColorMap(combined_feature_map, cv2.COLORMAP_JET)

            # 创建图像并保存
            plt.imshow(combined_feature_map, cmap=plt.cm.jet)  # cmap='gray' 可选
            plt.axis('off')
            plt.title(f'Stage {stage} Combined Feature Map')
            plt.savefig(f, dpi=300, bbox_inches='tight')
            plt.close()

            LOGGER.info(f'Saving {f}... (Combined Feature Map)')